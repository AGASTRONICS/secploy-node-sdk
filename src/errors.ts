...
import {
  ParsedError,
  StackFrame,
  parseError as parsePortableError,
  parseStack as parsePortableStack,
} from "./portable/stack";
...
function toModule(filename: string, root: string): string {
...
export function parseStack(stack: string, root = appRoot()): StackFrame[] {
  return parsePortableStack(stack, (filename) => toModule(filename, root));
}

/** Everything above, for one thrown value. */
export function parseError(thrown: unknown, root = appRoot()): ParsedError {
  return parsePortableError(thrown, (filename) => toModule(filename, root));
}
