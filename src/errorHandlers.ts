 ...
 * headers.
 */
function requestContext(req: RequestLike | undefined): Record<string, unknown> {
  if (!req) return {};
 ...
 */
export function expressErrorHandler(client: ErrorReporter) {
  return function secployErrorHandler(
    error: unknown,
    req: RequestLike,
    _res: unknown,
 ...
 */
export function fastifyErrorHandler(client: ErrorReporter) {
  return function secployErrorHandler(
    request: RequestLike,
    _reply: unknown,
    error: unknown,
