 ...
 */
export function classifyStatus(statusCode?: number | null): DeliveryOutcome {
  if (statusCode === null || statusCode === undefined) return "retry";
  ...
  if (RETRYABLE_CLIENT_ERRORS.has(code)) return "retry";
...
export function backoffDelay(
  ...
  const ceiling = Math.min(
    INITIAL_BACKOFF_MS * 2 ** Math.min(exponent, 10),
    MAX_BACKOFF_MS,
  );
