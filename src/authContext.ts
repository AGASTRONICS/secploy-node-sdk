...
export function normalizeAuthContext(
  input?: SecurityGateAuthContext | Record<string, any> | null,
  ...
  if (normalized.sessionId !== undefined && normalized.sessionId !== "") {
    normalized.sessionId = hashSessionId(normalized.sessionId);
  }

  return normalized as SecurityGateAuthContext;
}
