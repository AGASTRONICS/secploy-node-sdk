...
const withActor = (identity: string) => ({ context: { identity_key: identity } });
...
describe("protected types", () => {
  it("never samples errors", () => {
    ...
    for (const type of ["error", "critical", "fatal", "warning", "warn", "exception"]) {
  ...
  it("never samples security signals", () => {
    const signals = [
      "auth.anomaly.detected", "account.takeover.suspected", "security.threat.detected",
      "access.privilege_escalation.detected", "data.exfiltration.suspected",
      "secret.exposed", "incident.opened", "fraud.rule.matched", "payment.declined",
      "compliance.violation", "api.abuse.detected", "dependency_scan.completed",
    ];
  ...
  it("always sends anything carrying a stacktrace", () => {
    ...
    expect(shouldSend("info", { context: { stacktrace: ["File ..."] } }, 0.001)).toBe(true);
  });

  it("leaves volume traffic eligible", () => {
    // Without this, sampling does nothing at all.
    for (const type of ["log", "info", "debug", "metric", "http_request", "system_metrics"]) {
...
describe("actorKey", () => {
  it("prefers the most specific identifier", () => {
    expect(actorKey({ context: { identity_key: "u1", user_id: "u1", session_id: "s", ip_address: "1.2.3.4" } }))
      .toBe("identity_key:u1");
    expect(actorKey({ context: { session_id: "s", ip_address: "1.2.3.4" } })).toBe("session_id:s");
    expect(actorKey({ context: { ip_address: "1.2.3.4" } })).toBe("ip_address:1.2.3.4");
  });
  ...
  it("does not treat placeholders as actors", () => {
    ...
    expect(actorKey({ context: { identity_key: "anonymous", ip_address: "unknown" } })).toBe("");
  });
  ...
  it("yields nothing when there is nothing usable", () => {
    for (const payload of [{}, null, undefined, { context: "not an object" } as any]) {
...
describe("bucket", () => {
  ...
  it("spreads evenly", () => {
    ...
    for (let i = 0; i < 10000; i++) deciles[Math.floor(bucket(`identity_key:user-${i}`) * 10)]++;
...
describe("the event boundary", () => {
  function handlerWith(rate: number) {
    ...
    return { queue, handler: new EventHandler(queue, undefined, undefined, rate) };
  }
  ...
  it("actually applies sampling now", () => {
    ...
    for (let i = 0; i < 50; i++) {
      handler.sendEvent("log", { message: `line ${i}`, context: { identity_key: `u${i}` } });
    }
  ...
  it("still lets errors through at any rate", () => {
    const { queue, handler } = handlerWith(0);
    handler.sendEvent("error", { message: "boom", context: { identity_key: "u1" } });
  ...
  it("changes nothing at a full rate", () => {
    ...
    for (let i = 0; i < 20; i++) handler.sendEvent("log", { message: `line ${i}` });
  ...
  it("counts drops rather than losing them silently", () => {
    ...
    for (let i = 0; i < 200; i++) {
      handler.sendEvent("log", { message: "x", context: { identity_key: `u${i}` } });
    }
